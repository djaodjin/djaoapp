    ...
    var render = function (input, options, callback) {
        ...
        if (!callback) {
            ...
            this.parse(input, options, function(err, root, imports, options) {
                if (err) { console.log("XXX 1:", err); return callback(err); }
                ...
                catch (err) { console.log("XXX 2:", err); return callback(err); }
...
/* Turns off node visibility - if called node will NOT be shown in output
...
Ruleset.prototype.variables = function () {
    if (!this._variables) {
        this._variables = !this.rules ? {} : this.rules.reduce(function (hash, r) {
            ...
            /* when evaluating variables in an import statement, imports have
               not been eval'd so we need to go inside import statements.
...
Ruleset.prototype.genCSS = function (context, output) {
    ...
    function isRulesetLikeNode(rule) {
        /* if it has nested rules, then it should be treated like a ruleset
           medias and comments do not have nested rules, but should be treated
           like rulesets anyway some directives and anonymous nodes are ruleset
           like, others are not */
...
Ruleset.prototype.joinSelector = function (paths, context, selector) {
    ...
    function addReplacementIntoPath(beginningPath, addPath, replacedElement, originalSelector) {
        ...
        /* construct the joined selector - if & is the first thing this will
           be empty, if not newJoinedSelector will be the last set of elements
           in the selector */
